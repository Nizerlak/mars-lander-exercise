    ...
    fn from(value: &simulation::FlightState) -> Self {
        ...
        match value {
            ...
            FS::Landed(landing) => match landing {
                ...
                L::WrongTerrain { dist } => Self::CrashedWrongTerrain(*dist),
...
async fn main() {
    ...
    let app = match App::try_from_files(&sim_file_path, &settings_file_path) {
        Ok(app) => AppState {
    ...
    let router = Router::new()
        ...
        .route(
            ...
            put(|State(state): State<AppState>| async move {
                ...
                *app = App::try_from_files(&sim_file_path.clone(), &settings_file_path.clone())
                    .unwrap();
                app.run().unwrap();
            }),
        )
